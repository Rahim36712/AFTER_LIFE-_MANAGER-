...
DROP DATABASE IF EXISTS DigitalAfterlifeManager;
...
CREATE TABLE LEGAL_AUTHORITY (
    ...
    TRIGGER_EVENT_ID    INT(11),
    PRIMARY KEY (ID),
    FOREIGN KEY (TRIGGER_EVENT_ID) REFERENCES TRIGGER_EVENT(ID) ON DELETE SET NULL
);
...
-- Base entity for all digital asset types (complete, disjoint)
-- =============================================================
...
CREATE TABLE FINANCIAL_ASSET (
    Digital_Asset_ID    INT(11)         NOT NULL,
    AccountNumber       INT(11)         NOT NULL UNIQUE,
...
CREATE TABLE CLOUD_STORAGE_ASSET (
    ...
    StorageSize         INT(11)         NOT NULL COMMENT 'Storage size in MB',
...
-- =============================================================
-- TABLE: DIGITAL_ASSET_BENEFICIARY (Many-to-Many: Asset <-> Beneficiary)
-- =============================================================
...
CREATE TABLE ASSET_TRANSFER (
    ...
    Legal_Authority_ID  INT(11),
    PRIMARY KEY (ID),
    FOREIGN KEY (Digital_Asset_ID)  REFERENCES DIGITAL_ASSET(ID)    ON DELETE CASCADE,
    FOREIGN KEY (BeneficiaryID)     REFERENCES BENEFICIARY(ID)       ON DELETE CASCADE,
    FOREIGN KEY (TRIGGER_EVENT_ID)  REFERENCES TRIGGER_EVENT(ID)     ON DELETE CASCADE,
    FOREIGN KEY (Legal_Authority_ID) REFERENCES LEGAL_AUTHORITY(ID)  ON DELETE SET NULL
);

-- =============================================================
-- TABLE: ACCESS_LOG (Weak Entity — depends on USER)
-- =============================================================
...
-- =============================================================
-- TABLE: MEMORY_DATA (Weak Entity — depends on USER)
-- Stores personal memories/legacy data attached to a user
-- =============================================================
...
-- =============================================================
-- TABLE: ACCESS_POLICY_ASSET (Links ACCESS_POLICY to DIGITAL_ASSET)
-- =============================================================
ALTER TABLE DIGITAL_ASSET
ADD COLUMN Access_Policy_ID INT(11),
ADD FOREIGN KEY (Access_Policy_ID) REFERENCES ACCESS_POLICY(ID) ON DELETE SET NULL;
...
-- Index on DIGITAL_ASSET type and user for filtered queries
CREATE INDEX idx_asset_user ON DIGITAL_ASSET(USER_ID);
CREATE INDEX idx_asset_type ON DIGITAL_ASSET(Type);

-- Index on ASSET_TRANSFER status for pending-transfer reports
CREATE INDEX idx_transfer_status ON ASSET_TRANSFER(Status);

-- Index on TRIGGER_EVENT status for verification workflows
CREATE INDEX idx_trigger_status ON TRIGGER_EVENT(Status);

-- Index on ACCESS_LOG user for audit trail lookups
...
-- View 1: Pending asset transfers with full details
...
-- View 2: Full asset inventory per user
...
-- View 3: Beneficiary access summary
...
-- Trigger 1: Automatically log every new USER registration
CREATE TRIGGER trg_LogUserRegistration
AFTER INSERT ON USER
FOR EACH ROW
BEGIN
    INSERT INTO ACCESS_LOG (Action, Timestamp, USER_ID)
    VALUES (CONCAT('User registered: ', NEW.Name), NOW(), NEW.ID);
END$$

-- Trigger 2: When a TRIGGER_EVENT is verified, automatically
--            update all related ASSET_TRANSFERs to 'Completed'
--            and set their Transfer_Date
CREATE TRIGGER trg_AutoCompleteTransfers
AFTER UPDATE ON TRIGGER_EVENT
FOR EACH ROW
BEGIN
    IF NEW.Status = 'Verified' AND OLD.Status != 'Verified' THEN
        ...
        -- Also log the trigger verification
        INSERT INTO ACCESS_LOG (Action, Timestamp, USER_ID)
        VALUES (
            CONCAT('Trigger event verified: ', NEW.EventType, '. Transfers completed.'),
            NOW(),
            NEW.USER_ID
        );
    END IF;
...
-- Trigger 3: Prevent deletion of a DIGITAL_ASSET that has pending transfers
CREATE TRIGGER trg_PreventAssetDeletion
BEFORE DELETE ON DIGITAL_ASSET
FOR EACH ROW
BEGIN
    DECLARE pending_count INT;
    SELECT COUNT(*) INTO pending_count
    FROM ASSET_TRANSFER
    WHERE Digital_Asset_ID = OLD.ID AND Status = 'Pending';

    IF pending_count > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Cannot delete asset with pending transfers.';
    END IF;
END$$
...
('Ahmed Khan',       'ahmed.khan@email.com',       SHA2('pass123', 256),  '0300-1234567', '1985-03-15'),
('Sara Malik',       'sara.malik@email.com',        SHA2('sara456', 256),  '0311-2345678', '1990-07-22'),
('Bilal Hussain',    'bilal.h@email.com',           SHA2('bilal789', 256), '0333-3456789', '1978-11-05'),
('Hina Noor',        'hina.noor@email.com',         SHA2('hina321', 256),  '0321-4567890', '1995-01-30'),
('Omar Farooq',      'omar.farooq@email.com',       SHA2('omar654', 256),  '0345-5678901', '1982-09-14'),
('Fatima Zahra',     'fatima.z@email.com',          SHA2('fatima99', 256), '0301-6789012', '1988-06-18'),
('Usman Ali',        'usman.ali@email.com',         SHA2('usman11', 256),  '0312-7890123', '1975-12-25'),
('Zainab Rauf',      'zainab.r@email.com',          SHA2('zainab22', 256), '0322-8901234', '1992-04-08'),
('Tariq Mehmood',    'tariq.m@email.com',           SHA2('tariq33', 256),  '0344-9012345', '1968-08-20'),
('Amna Sheikh',      'amna.sheikh@email.com',       SHA2('amna44', 256),   '0302-0123456', '1997-02-14');
...
-- Financial Asset Specializations
...
-- Digital Asset <-> Beneficiary Assignments (Many-to-Many)
INSERT INTO DIGITAL_ASSET_BENEFICIARY (Digital_Asset_ID, BeneficiaryID) VALUES
(1,  1), (1,  2),   -- HBL Account -> Ali Khan, Sana Malik
(2,  1),            -- Facebook    -> Ali Khan
(3,  3),            -- Google Drive -> Hassan Hussain
(4,  4), (4,  5),   -- Meezan Bank  -> Maryam, Imran
(5,  4),            -- Instagram    -> Maryam
(6,  5),            -- Dropbox      -> Imran
(7,  6),            -- UBL Account  -> Nadia
(8,  7),            -- Twitter      -> Kamran
(9,  8),            -- OneDrive     -> Rabia
(10, 9), (10, 1),   -- Allied Bank  -> Danish, Ali Khan
(11, 3),            -- LinkedIn     -> Hassan
(12, 4),            -- iCloud       -> Maryam
(13, 5), (13, 6),   -- MCB Account  -> Imran, Nadia
(14, 7),            -- YouTube      -> Kamran
(15, 8);            -- Mega         -> Rabia
...
-- Additional Access Logs (Manual entries)
...
-- =============================================================
-- SECTION 6: SQL QUERIES (15+ as Required by CEP)
-- =============================================================

-- ---------------------------------------------------------------
-- QUERY 1: List all users with their total asset count and total value
...
-- ---------------------------------------------------------------
-- QUERY 2: Top 5 users by financial asset value
-- (JOIN + ORDER BY + LIMIT)
-- ---------------------------------------------------------------
SELECT
    u.Name          AS AssetOwner,
...
-- ---------------------------------------------------------------
-- QUERY 3: All pending transfers with owner, beneficiary, and asset info
-- (Multi-table JOIN)
-- ---------------------------------------------------------------
SELECT
    at.ID           AS TransferID,
...
-- ---------------------------------------------------------------
-- QUERY 4: Beneficiaries assigned to more than one digital asset
-- (HAVING clause)
-- ---------------------------------------------------------------
SELECT
    b.ID,
...
-- ---------------------------------------------------------------
-- QUERY 5: Detect users with NO beneficiary assigned to any asset
-- (Subquery / NOT IN)
-- ---------------------------------------------------------------
SELECT
    u.ID,
...
-- ---------------------------------------------------------------
-- QUERY 6: Monthly trend of trigger events in 2025-2026
...
FROM TRIGGER_EVENT
GROUP BY YEAR(EventDate), MONTH(EventDate), MONTHNAME(EventDate)
ORDER BY Year, Month;

-- ---------------------------------------------------------------
-- QUERY 7: Identify high-value assets (above average value)
-- (Subquery in WHERE)
-- ---------------------------------------------------------------
SELECT
    da.ID,
...
-- ---------------------------------------------------------------
-- QUERY 8: Full activity audit trail for a specific user (USER_ID = 1)
-- (JOIN + ORDER BY timestamp)
-- ---------------------------------------------------------------
SELECT
    al.ID,
    al.Action,
...
-- ---------------------------------------------------------------
-- QUERY 9: Assets per type with average value and total count
...
-- ---------------------------------------------------------------
-- QUERY 10: Beneficiaries who have NEVER received a completed transfer
-- (NOT IN subquery)
-- ---------------------------------------------------------------
SELECT
    b.ID,
...
-- ---------------------------------------------------------------
-- QUERY 11: Cloud storage assets with more than 3000 files
-- (JOIN specialization table + WHERE)
-- ---------------------------------------------------------------
SELECT
    da.Name         AS AssetName,
    u.Name          AS Owner,
...
-- ---------------------------------------------------------------
-- QUERY 12: Transfer completion rate per legal authority
-- (JOIN + CASE + GROUP BY)
-- ---------------------------------------------------------------
SELECT
    la.Name             AS LegalAuthority,
...
-- ---------------------------------------------------------------
-- QUERY 13: Users who have uploaded memory data AND have pending transfers
-- (Complex logical condition — nested subqueries + IN)
-- ---------------------------------------------------------------
SELECT DISTINCT
    u.ID,
    u.Name,
...
-- ---------------------------------------------------------------
-- QUERY 14: Summary of each user's digital estate
-- (Multi-join aggregation across specialization tables)
-- ---------------------------------------------------------------
SELECT
    u.Name                  AS UserName,
    COUNT(da.ID)            AS TotalAssets,
...
-- ---------------------------------------------------------------
-- QUERY 15: Detect assets with no access policy assigned
-- (LEFT JOIN + IS NULL)
-- ---------------------------------------------------------------
SELECT
    da.ID,
...
-- ---------------------------------------------------------------
-- QUERY 16: List all social media assets with owner and beneficiary info
-- (3-way JOIN through specialization)
-- ---------------------------------------------------------------
SELECT
    u.Name          AS OwnerName,
    sma.PlatformName,
...
-- ---------------------------------------------------------------
-- QUERY 17: Access log frequency per user (last 90 days)
-- (Date filter + GROUP BY + HAVING)
-- ---------------------------------------------------------------
SELECT
    u.ID,
    u.Name,
...
FROM ACCESS_LOG al
JOIN USER u ON al.USER_ID = u.ID
WHERE al.Timestamp >= DATE_SUB(NOW(), INTERVAL 90 DAY)
GROUP BY u.ID, u.Name
HAVING COUNT(al.ID) >= 1
ORDER BY AccessCount DESC;

-- ---------------------------------------------------------------
-- QUERY 18: Retrieve pending transfers with a verified trigger event
--           (Logical inconsistency detection — data integrity check)
-- ---------------------------------------------------------------
SELECT
    at.ID           AS TransferID,
    da.Name         AS AssetName,
...
-- UPDATE: Mark a trigger event as Verified (this fires the trigger)
UPDATE TRIGGER_EVENT
SET Status = 'Verified'
WHERE ID = 5 AND Status = 'Pending';

-- UPDATE: Change a beneficiary's access policy
UPDATE BENEFICIARY
SET Access_Policy_ID = 1
WHERE ID = 4;

-- UPDATE: Correct a bank balance
UPDATE FINANCIAL_ASSET
SET Balance = 280000
WHERE Digital_Asset_ID = 1;

-- DELETE: Remove a rejected trigger event (no pending transfers on it)
...
VALUES ('Hamza Baig', 'hamza.baig@email.com', SHA2('hamzapass', 256), '0303-1122334', '2000-06-06');

-- INSERT: Add a new digital asset for Hamza
INSERT INTO DIGITAL_ASSET (Name, Type, Value, USER_ID)
VALUES ('JS Bank Account', 'Financial', 95000, LAST_INSERT_ID());

-- INSERT: Add specialization for the new asset
INSERT INTO FINANCIAL_ASSET (Digital_Asset_ID, AccountNumber, BankName, Balance)
VALUES (LAST_INSERT_ID(), 99988877, 'JS Bank', 95000);
